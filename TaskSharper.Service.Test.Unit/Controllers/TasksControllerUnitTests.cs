...
namespace TaskSharper.Service.Test.Unit.Controllers
{
    public class TasksControllerUnitTests
    {
        ...
        public async Task GetWithId_IdSuppliedButReturnsEventWithNoneEventType_Return404()
        {
            ...
            Assert.That(data?.StatusCode, Is.EqualTo(HttpStatusCode.NotFound));
        }

        [Test]
        public async Task GetWithId_IdSuppliedButReturnsEventWithAppointmentEventType_Return404()
        {
            _eventManager.GetEventAsync(Arg.Any<string>(), Arg.Any<string>()).Returns(new Event()
            {
        ...
        public async Task GetWithBetweenDates_ValidTimeSpan_ReturnListOfEvents()
        {
            ...
            Assert.That(result, Is.TypeOf<OkNegotiatedContentResult<IEnumerable<Event>>>());
        }

        [Test]
        public async Task GetWithBetweenDates_ValidTimeSpanReturnsAListOfMultipleDifferentEventTypesWhereOnly3IsOfTypeTask_ReturnListWith2Events()
        {
            _eventManager.GetEventsAsync(Arg.Any<DateTime>(), Arg.Any<DateTime>()).Returns(new List<Event>()
            {
                new Event()
                {
                    Type = EventType.Task
            ...
            var data = result as OkNegotiatedContentResult<IEnumerable<Event>>;

            Assert.That(data?.Content.Count(), Is.EqualTo(3));
        }
